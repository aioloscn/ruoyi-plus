...
public class DefaultResolver implements Resolver {
    ...
    private String processValue(String sourceValue,SpiderField field){
        ...
        for(SpiderFiledRule rule:rules){
            ...
            } else if (SpiderConstants.FILED_PROCESS_TYPE_RETAIN.equals(rule.getProcessType())) {
                // 保留匹配正则的内容
                Pattern compile = Pattern.compile(rule.getReplacereg());
                Matcher matcher = compile.matcher(sourceValue);
                String group = StringUtils.EMPTY;
                while (matcher.find()) {
                    group += matcher.group() + StringUtils.SPACE;
                }
                sourceValue = group;
            } else{}
        }
