...
(function ($) {
    $.extend({
        ...
        operate: {
			...
			// 从Redis同步
			syncFromRedis: function (dictType) {
				$.modal.confirm("要从Redis同步到数据库吗？请提前备份好数据", function () {
					var url = table.options.syncFromRedis;
					$.operate.submit(url, "post", "json", {"dictType": dictType});
				});
			},
			// 同步到Redis
			syncToRedis: function (dictType) {
				$.modal.confirm("要同步数据到Redis吗？", function () {
					var url = table.options.syncToRedis;
					$.operate.submit(url, "post", "json", {"dictType": dictType});
				});
			}
        },
