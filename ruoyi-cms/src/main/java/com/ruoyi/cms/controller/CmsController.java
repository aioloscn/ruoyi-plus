...
import com.ruoyi.cms.resources.FileResource;
        ...
        if (file != null) {
            ...
            if (StringUtils.isNotNull(filename)) {
                ...
                if (!suffix.equalsIgnoreCase("png") && !suffix.equalsIgnoreCase("jpg")
                    ...
                    return AjaxResult.error(ErrorEnum.FILE_FORMAT_ERROR.getErrMsg());
                }

                // 上传到OSS
                try {
                    path = cms.uploadOSS(file, suffix);
                } catch (IOException e) {
                    return AjaxResult.error(ErrorEnum.FILE_UPLOAD_FAILED.getErrMsg());
                }
            } else {
                return AjaxResult.error(ErrorEnum.FILE_UPLOAD_NULL_ERROR.getErrMsg());
            }
        } else {
            return AjaxResult.error(ErrorEnum.FILE_UPLOAD_NULL_ERROR.getErrMsg());
        }
...
//        String url = serverConfig.getUrl() + path;
        String url = fileResource.getPrefix() + path;
        Map<String,Object> data=new HashMap();
        int width=FileUploadUtils.getImgWidth(file);
        int height=FileUploadUtils.getImgHeight(file);
        data.put("path",url);
